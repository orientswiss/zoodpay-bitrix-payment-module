...
if ($_SERVER["REQUEST_METHOD"] == "POST" && !empty($safePost["refund_id"]) && !empty($safePost["signature"]) && !empty($safePost["refund"]['transaction_id'])) {
    ...
    $transaction = $safePost["refund"]["transaction_id"];
    $LocalTransaction = $zDataHelper->getTotalRowData($zDataHelper::ZOODPAY_TRANSACTIONS_TABLE, $zDataHelper::ZOODPAY_TRANSACTIONS_id, $transaction);
