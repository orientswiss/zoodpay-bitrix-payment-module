...
if ($_SERVER["REQUEST_METHOD"] == "POST" && !empty($safePost["merchant_order_reference"]) && !empty($safePost["transaction_id"]) && !empty($safePost["amount"]) && !empty($safePost["status"]) && !empty($safePost["signature"])) {
    ...
    if(isset($LocalTransaction)) {
        ...
        if ($localSignature == $safePost["signature"]) {
            ...
            switch ($safePost["status"]) {
                ...
                case  "Failed" :
                {
                    ...
                    $arFields = array(
                        ...
                        "PS_STATUS_CODE" => $merchantData['FAILED_STATUS'],
                        "PS_STATUS_DESCRIPTION" => $safePost["status"],
                        "PS_STATUS_MESSAGE" => ("The ZoodPay ID for this transaction: " . $safePost["transaction_id"] . ", and transaction is Failed " .  $safePost["created_at"]),
                        "PS_SUM" => 0,
                        "PS_CURRENCY" => $LocalTransaction['currency'],
                        "PS_RESPONSE_DATE" => Date(CDatabase::DateFormatToPHP(CLang::GetDateFormat("FULL", LANG))),
                        "USER_ID" => $arOrder["USER_ID"]
                ...
                case  "Cancelled" :
                {
                    $arFields = array(
                        ...
                        "CANCELED" => "Y",
                        "PS_STATUS_DESCRIPTION" => $safePost["status"],
                        "PS_STATUS_MESSAGE" => ("The ZoodPay ID for this transaction: " . $safePost["transaction_id"] . ", and transaction is Cancelled " .  $safePost["created_at"]),
                        "PS_SUM" => 0,
                        "PS_CURRENCY" => $LocalTransaction['currency'],
                        "PS_RESPONSE_DATE" => Date(CDatabase::DateFormatToPHP(CLang::GetDateFormat("FULL", LANG))),
                        "USER_ID" => $arOrder["USER_ID"]
            ...
            echo "Payment Status Changed to " . $safePost["status"];
        }
